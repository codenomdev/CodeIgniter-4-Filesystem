...
class Filesystem
{
     ...
     */
    public function get($path, $flag = null, $context = null, $lock = false)
    {
        if ($this->isFile($path)) {
            return $lock ? $this->sharedGet($path) : file_get_contents($path, $flag, $context);
        }

        throw FilesystemException::FileNotFoud($path);
    }
    ...
    // {
    //     if (!$this->isFile($path)) {
    //         throw FilesystemException::FileNotFoud(
    //             $path
    //         );
    //     }
     ...
     */
    public function fileOpen(string $path, string $mode = 'r')
    {
        $result = @fopen($path, $mode);

        if (!$result) {
            throw FilesystemException::FileNotWritable($path);
        }
    ...
    public function fileRead($resource, $length)
    {
        ...
        if (!$result) {
            throw FilesystemException::FileNotReadable();
        }
    ...
    public function fileGetCsv($resource, $length = 0, $delimiter = ',', $enclosure = '"', $escape = '\\')
    {
        ...
        if ($result === null) {
            throw FilesystemException::WrongCSVHandle();
        }
        return $result;
    }
    ...
    public function fileTell($resource)
    {
        ...
        if ($result === null) {
            throw FilesystemException::FileTellError();
        }
        return $result;
    }
    ...
    public function fileSeek($resource, $offset, $whence = SEEK_SET)
    {
        ...
        if ($result === -1) {
            throw FilesystemException::FileSeekError();
        }
        return $result;
    }
    ...
    public function fileClose($resource)
    {
        ...
        if (!$result) {
            throw FilesystemException::FileCloseError();
        }
        return $result;
    }
    ...
    public function stat($path)
    {
        ...
        $result = @stat($path);
        if (!$result) {
            throw FilesystemException::StatisticError();
        }
        return $result;
    }
